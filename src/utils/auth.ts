...
export const updateUserLikePlaylist = async () => {
  ...
  // 创建包装函数以适配 setUserLikeDataLoop 的接口
  const playlistApiWrapper = (limit: number, offset: number) =>
    userPlaylist(limit, offset, userId);

  await setUserLikeDataLoop(playlistApiWrapper, formatCoverList, "playlists");
};
...
const setUserLikeDataLoop = async <T>(
  ...
  let offset = 0;
  const allData: T[] = [];
  const limit = 50; // 网易云 API 限制每页最多50条
